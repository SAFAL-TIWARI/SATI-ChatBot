# SATI AI Assistant - Comprehensive Knowledge Base
...
def get_contextual_prompt(user_query):
    """
    Returns appropriate context based on the user's query about SATI
    """
    ...
    if any(
        word in query_lower
        for word in ["history", "established", "founded", "background", "ashoka"]
    ):
        relevant_contexts.append(SATI_KNOWLEDGE_BASE["history"]["content"])
    ...
    if any(
        ...
        relevant_contexts.append(SATI_KNOWLEDGE_BASE["academic_programs"]["content"])

    if any(
        word in query_lower
        for word in ["hostel", "accommodation", "facility", "campus", "library"]
    ):
        relevant_contexts.append(SATI_KNOWLEDGE_BASE["campus_facilities"]["content"])

    if any(
        word in query_lower
        for word in ["placement", "job", "salary", "company", "career"]
    ):
        relevant_contexts.append(SATI_KNOWLEDGE_BASE["placements"]["content"])

    if any(
        word in query_lower
        for word in ["club", "activity", "sports", "student life", "ncc", "nss"]
    ):
        relevant_contexts.append(SATI_KNOWLEDGE_BASE["student_life"]["content"])

    if any(
        word in query_lower
        for word in ["alumni", "kailash satyarthi", "notable", "famous"]
    ):
        relevant_contexts.append(SATI_KNOWLEDGE_BASE["notable_alumni"]["content"])

    # If no specific context matches, provide general overview
    if not relevant_contexts:
        relevant_contexts.append(SATI_KNOWLEDGE_BASE["institute_overview"]["content"])

    # Combine relevant contexts
    context = "\n\n".join(relevant_contexts)

    return f"{SATI_AI_PROMPT}\n\nRELEVANT CONTEXT FOR YOUR QUERY:\n{context}\n\nUser Query: {user_query}\n\nPlease provide a helpful and accurate response based on the above information."


# Default prompt for general queries
def get_default_prompt():
    """Returns the default SATI AI prompt"""
